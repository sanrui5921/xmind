...
public class Solution_0725 {
    ...
    public ListNode[] splitListToParts(ListNode root, int k) {
        ...
        for (int i = 0; i < k; i++) {
            sizes[i] = i < remainder ? divisor + 1 : divisor;
        }

        ListNode[] results = new ListNode[k];

        ListNode[] currents = new ListNode[k];

        int i = 0;

        for (ListNode current = root; current != null; i++) {
            for (int j = 0; j < sizes[i]; j++, current = current.next) {
                if (currents[i] == null) {
                    results[i] = current;
                }
                currents[i] = current;
